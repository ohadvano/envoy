...
FcdsApiPtr ListenerImpl::createFcdsSubscription(
  ...
  std::unique_ptr<FcdsApiImpl> subscription = std::make_unique<FcdsApiImpl>(
      fcds_config.config_source(), !fcds_config.name().empty() ? fcds_config.name() : name_, name_,
...
ListenerImpl::ListenerImpl(ListenerImpl& origin, const FilterChainRefVector& added_filter_chains,
  ...
  // init objects so new listener will only be initialized after all previous version dependencies
  // are also initialized.
  if (origin.dynamic_init_manager_->state() == Init::Manager::State::Initializing) {
    parents_dynamic_init_manager_ = origin.parents_dynamic_init_manager_;
    parents_dynamic_init_manager_.push_back(origin.dynamic_init_manager_);
    parents_listener_init_target_ = origin.parents_listener_init_target_;
    parents_listener_init_target_.push_back(origin.listener_init_target_);
    parents_local_init_watcher_ = origin.parents_local_init_watcher_;
    parents_local_init_watcher_.push_back(origin.local_init_watcher_);
  }
  ...
  } else if (origin.dynamic_init_manager_->state() == Init::Manager::State::Initializing) {
    tracking_init_target_ = std::make_unique<Init::TargetImpl>("origin-listener-tracking-target", []() {});
    tracking_init_watcher_ = std::make_unique<Init::WatcherImpl>("origin-listener-tracking-watcher", [&]() {
        tracking_init_target_->ready();
        tracking_init_target_.reset();
    });
...
ListenerImpl::getDiscoveredFilterChainsConfig(const Matchers::StringMatcher& name_matcher) const {
  return filter_chain_manager_->getDiscoveredFilterChainsConfig(name_matcher);
}
