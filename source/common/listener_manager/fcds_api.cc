...
FcdsApiImpl::FcdsApiImpl(const envoy::config::core::v3::ConfigSource& fcds_config,
    ...
    : Envoy::Config::SubscriptionBase<envoy::config::listener::v3::FilterChain>(validation_visitor,
      ...
      local_init_target_("FCDS-local", [this]() {
...
FcdsApiImpl::onConfigUpdate(const std::vector<Config::DecodedResourceRef>& added_resources,
  ...
  listener_manager_.beginFilterChainsUpdate(listener_name_);
  ...
  TRY_ASSERT_MAIN_THREAD {
    absl::Status update_or_error =
        listener_manager_.updateDynamicFilterChains(listener_name_, version, added_filter_chains, removed_filter_chains);
  ...
  absl::optional<ListenerManager::FailureState> failure_state;
  if (!error_message.empty()) {
    failure_state.emplace(ListenerManager::FailureState());
    failure_state->set_details(error_message);
  }

  listener_manager_.endFilterChainsUpdate(listener_name_, failure_state);
  init_target_.ready();
