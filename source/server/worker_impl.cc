...
std::unique_ptr<ConnectionHandler> getHandler(Event::Dispatcher& dispatcher, uint32_t index,
  ...
  if (factory) {
    return factory->createConnectionHandler(dispatcher, index, overload_manager, listener_manager,
...
WorkerPtr ProdWorkerFactory::createWorker(uint32_t index, OverloadManager& overload_manager,
  ...
  auto conn_handler =
      getHandler(*dispatcher, index, listener_manager, overload_manager, null_overload_manager);
